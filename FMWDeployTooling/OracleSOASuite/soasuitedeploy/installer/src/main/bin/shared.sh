...
#
#     NAME
#       shared.sh - shared script for use with SOASuite Deploy Tooling.
#
#     DESCRIPTION
#       This script contains shared functions for use with SOASuite Deploy Tooling scripts.
#
...
variableSetup() {
    ...
    if [ -z "${WLSDEPLOY_HOME}" ]; then
		if [ ! -d "${BASEDIR}/../../weblogic-deploy" ]; then
			echo "Could not find WebLogic Deploy tooling. FMW deploy requires WebLogic Deploy tooling. Extract WDT in the same folder as fmw deploy." >&2
			exit 2
		fi
        WLSDEPLOY_HOME=`cd "${BASEDIR}/../../weblogic-deploy" ; pwd`
	...
	SOASUITEDEPLOY_HOME=`cd "${BASEDIR}/.."; pwd`
	WDT_CUSTOM_CONFIG=${SOASUITEDEPLOY_HOME}
	export WDT_CUSTOM_CONFIG
	export SOASUITEDEPLOY_HOME
...
runWlst() {
	...
	echo "SOASUITEDEPLOY_HOME = ${SOASUITEDEPLOY_HOME}"

    PY_SCRIPTS_PATH="${SOASUITEDEPLOY_HOME}/lib/python"
