...
#
# Description
#  This sample script to upgrade OracleIdentityGovernance domain

# Initialize
script="${BASH_SOURCE[0]}"
base_script="$(basename "${BASH_SOURCE[1]}" .sh)"
...
#
# Function to capture domain-upgrade.sh script execution log
#
...
createScriptConfigmap() {
   ...
   ProductScriptsConfigmapDir=${scriptDir}/governancedomain-domain-upgrade
...
${KUBERNETES_CLI:-kubectl} cp "$namespace/$podName:/u01/oracle/user_projects/domains/upgrade_$domainUID/logs" "${domainUpgradeOutputDir}/logs/" --retries=3
