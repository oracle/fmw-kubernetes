...
#  2) oimcluster-weblogic-credentials - if secretName and domainUID are both not specified. This is the default out-of-the-box.
...
function usage {
  ...
  echo "  -d domainUID, optional. The default value is oimcluster. If specified, the secret will be labeled with the domainUID unless the given value is an empty string."
  echo "  -n namespace, optional. Use the oimcluster namespace if not specified"
