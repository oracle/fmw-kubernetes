...
1. [Create OAM Domains Using WLST Scripts](#create-oam-domains-using-wlst-scripts)
...
If you want to create an OAM domain using WDT models, ignore everything else on this page and move directly to [Create OAM Domains Using WDT Models](../create-oam-domains/create-oam-domains-using-wdt-models).
...
1. If using Oracle Container Registry or your own container registry for your OAM container image, run the following command to create a helper pod to run RCU:
   ...
   ```bash
   $ kubectl run --image=container-registry.oracle.com/middleware/oam_cpu:12.2.1.4-jdk8-ol8-<January'26> --image-pull-policy="IfNotPresent" --overrides='{"apiVersion": "v1","spec":{"imagePullSecrets": [{"name": "orclcred"}]}}' helper -n oamns -- sleep infinity
   ```
   ...
   ```bash
   $ kubectl run helper --image oracle/oam:12.2.1.4-jdk8-ol8-<January'26> -n oamns -- sleep infinity
   ```
...
1. Run the following commands to verify the PV and PVC were created successfully:
   ...
   You are now ready to create the OAM domain with WLST scripts as as per [Create OAM Domains Using WLST scripts](../create-oam-domains/create-oam-domains-using-wlst).
