...
For the current production release 21.3.2:
...
* Docker 18.09.1ce, 19.03.1 (check with `docker version`) or CRI-O 1.17.0 (check with `crictl version | grep RuntimeVersion`).
...
* Oracle SOA Suite 12.2.1.4 Docker image downloaded from My Oracle Support (MOS patch [33125465](https://support.oracle.com/epmos/faces/ui/patch/PatchDetail.jspx?patchId=33125465)). This image contains the latest bundle patch and one-off patches for Oracle SOA Suite.
