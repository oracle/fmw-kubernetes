...
1. Download patch [32794257](https://support.oracle.com/epmos/faces/ui/patch/PatchDetail.jspx?patchId=32794257) from My Oracle Support (MOS).
...
1. Load the image archive using the `docker load` command.
   ...
   $
   ```
1. Run the `docker inspect` command to verify that the downloaded image is the latest released image. The value of label `com.oracle.weblogic.imagetool.buildid` must match to `32a1cc91-961d-4ce9-9d57-9f2eaff7bbea`.
...
The WebLogic Kubernetes Operator supports the deployment of Oracle SOA Suite domains in the Kubernetes environment. Follow the steps in [this document](https://github.com/oracle/weblogic-kubernetes-operator/blob/v3.2.1/documentation/3.2/content/quickstart/install.md) to install the operator.
