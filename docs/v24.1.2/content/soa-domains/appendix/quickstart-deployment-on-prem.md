...
#### 1.2 Install and configure CRI-O and Podman
...
1. Installing CRI-O:
    ...
    ```shell
    ### Installing the plugins and additional tools needed to setup CRI-O, namely flannel-cni-plugin, kubernetes-cni-plugins and cri-tools
    $ dnf install https://yum.oracle.com/repo/OracleLinux/OL8/olcne18/x86_64/getPackage/flannel-cni-plugin-1.2.0-3.el8.x86_64.rpm
    $ dnf install https://yum.oracle.com/repo/OracleLinux/OL8/olcne18/x86_64/getPackage/kubernetes-cni-plugins-1.2.0-6.el8.x86_64.rpm
    $ dnf install https://yum.oracle.com/repo/OracleLinux/OL8/olcne18/x86_64/getPackage/cri-tools-1.28.0-2.el8.x86_64.rpm

    ### Installing crio
    $ dnf install https://yum.oracle.com/repo/OracleLinux/OL8/olcne18/x86_64/getPackage/cri-o-1.28.2-2.el8.x86_64.rpm
    ```
...
1. Configure podman rootless:
    ...
    ```shell
    $ usermod --add-subuids 10000-65536 --add-subgids 10000-65536 <user-id>
...
1. From Kubernetes version v1.22 onward, `kubeadm` will default `cgroup-driver` to `systemd`. If your CRI-O is using cgroup driver as `cgroupfs`, set `--cgroup-driver=cgroupfs` for kubelet.
    ```shell
...
1. Install a pod network add-on (`flannel`) so that your pods can communicate with each other.
   ...
   ```shell
   $ wget https://raw.githubusercontent.com/flannel-io/flannel/v0.24.2/Documentation/kube-flannel.yml
...
1. Pull the WebLogic Kubernetes Operator image:
    ...
    ```shell
    $ podman pull ghcr.io/oracle/weblogic-kubernetes-operator:4.1.7
    ```
...
The WebLogic Kubernetes Operator v4.1.7 has been installed. Continue with the load balancer and Oracle SOA Suite domain setup.
