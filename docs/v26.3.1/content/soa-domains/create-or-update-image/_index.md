...
The installation binaries and patches required for release 26.3.1 are:

* JDK:  
    * jdk-8u501-linux-x64.tar.gz
...
* Fusion Middleware Infrastructure patches:
    ...
    * p28186730_1394224_Generic.zip (OPATCH 13.9.4.2.24 FOR EM 13.5/24.1 AND FMW/WLS 12.2.1.4.0, 14.1.1.0.0, 14.1.2.0.0 AND IDM 14.1.2.1)
    * p39039951_122140_Generic.zip (OWSM BUNDLE PATCH 12.2.1.4.260304)
    * p39597055_122140_Generic.zip (ADF BUNDLE PATCH 12.2.1.4.260622)
    * p39638312_122140_Generic.zip (WLS PATCH SET UPDATE 12.2.1.4.260625)
    * p34809489_122140_Generic.zip (JDEV One-off)
    * p36789759_122140_Generic.zip (FMW PLATFORM BUNDLE PATCH 12.2.1.4.240812)
    * p39659274_122140_Generic.zip (OPSS BUNDLE PATCH 12.2.1.4.260629)
    * p1221431_122140_Generic.zip (Coherence Cumulative Patch 12.2.1.4.31)
    * p37056593_122140_Generic.zip (Cloning One-off)
    * p39465265_122140_Generic.zip (WLS One-off)
    * p39376152_122140_Generic.zip (WEBCENTER CORE BUNDLE PATCH 12.2.1.4.260513)
    * p39491156_122140_Generic.zip (FMWCONTROL BUNDLE PATCH 12.2.1.4.260603)
    * p35965629_122140_Generic.zip (ADR FOR WEBLOGIC SERVER 12.2.1.4.0 - SIZE OPTIMIZED FOR JAN 2024)
    * p39246821_122140_Generic.zip (RDA release 26.3-2026721 for OFM 12.2.1.4 SPB)
    * p39487427_122140_Generic.zip (JDBC19.31 BUNDLE PATCH 12.2.1.4.260603)
    * p39574504_122140_Generic.zip (FMW Thirdparty Bundle Patch 12.2.1.4.260617)
    * p39567777_122140_Generic.zip (OSS 19C BUNDLE PATCH 12.2.1.4.260616)

* Oracle SOA Suite and Oracle Service Bus patches

    * p39703215_122130_Generic.zip (UMS (On Prem) Bundle Patch 12.2.1.3.260708)
    * p39671832_122140_Generic.zip (SOA Bundle Patch 12.2.1.4.260701)
    * p38829855_122140_Generic.zip (OSB Bundle Patch 12.2.1.4.260108)
...
1. Add a JDK package to the WebLogic Image Tool cache:
    ...
    ``` bash
    $ imagetool cache addInstaller --type jdk --version 8u501 --path <download location>/jdk-8u501-linux-x64.tar.gz
    ```
...
1. Add the downloaded OPatch patch to the WebLogic Image Tool cache:
    ...
    ``` bash
    $ imagetool cache addEntry --key 28186730_13.9.4.2.22 --value <download location>/p28186730_1394224_Generic.zip
    ```

1. Append the `--opatchBugNumber` flag and the OPatch patch key to the `create` command in the `buildArgs` file:

    ``` bash
    --opatchBugNumber 28186730_13.9.4.2.22
    ```

1. Add the downloaded product patches to the WebLogic Image Tool cache:  

    ``` bash   
	$ imagetool cache addEntry --key 39703215_12.2.1.3.0 --value <download location>/39703215_122130_Generic.zip

	$ imagetool cache addEntry --key 39039951_12.2.1.4.0 --value <download location>/39039951_122140_Generic.zip

	$ imagetool cache addEntry --key 39597055_12.2.1.4.0 --value <download location>/39597055_122140_Generic.zip

	$ imagetool cache addEntry --key 39638312_12.2.1.4.0 --value <download location>/39638312_122140_Generic.zip
	...
	$ imagetool cache addEntry --key 39659274_12.2.1.4.0 --value <download location>/39659274_122140_Generic.zip

	$ imagetool cache addEntry --key 1221431_12.2.1.4.0 --value <download location>/p1221431_122140_Generic.zip

	$ imagetool cache addEntry --key 37056593_12.2.1.4.0 --value <download location>/37056593_122140_Generic.zip

	$ imagetool cache addEntry --key 39465265_12.2.1.4.0 --value <download location>/39465265_122140_Generic.zip

	$ imagetool cache addEntry --key 38400138_12.2.1.4.0 --value <download location>/39376152_122140_Generic.zip

	$ imagetool cache addEntry --key 36946553_12.2.1.4.0 --value <download location>/39491156_122140_Generic.zip
	...
	$ imagetool cache addEntry --key 38199225_12.2.1.4.0 --value <download location>/39246821_122140_Generic.zip

	$ imagetool cache addEntry --key 37925688_12.2.1.4.0 --value <download location>/39487427_122140_Generic.zip

	$ imagetool cache addEntry --key 38405729_12.2.1.4.0 --value <download location>/39574504_122140_Generic.zip

	$ imagetool cache addEntry --key 38379810_12.2.1.4.0 --value <download location>/39567777_122140_Generic.zip

	$ imagetool cache addEntry --key 38253323_12.2.1.4.0 --value <download location>/39671832_122140_Generic.zip

	$ imagetool cache addEntry --key 38059281_12.2.1.4.0 --value <download location>/38829855_122140_Generic.zip
...
1. Append the `--patches` flag and the product patch keys to the `create` command in the `buildArgs` file. The `--patches` list must be a comma-separated collection of patch `--key` values used in the `imagetool cache addEntry` commands above.
   ...
   Sample `--patches` list for the product patches added in to the cache:
      ...
      ```
      --patches 39039951_12.2.1.4.0,39597055_12.2.1.4.0,39638312_12.2.1.4.0,34809489_12.2.1.4.0,36789759_12.2.1.4.0,39659274_12.2.1.4.0,1221431_12.2.1.4.0,37056593_12.2.1.4.0,39465265_12.2.1.4.0,33093748_12.2.1.4.0,39376152_12.2.1.4.0,39491156_12.2.1.4.0,34765492_12.2.1.4.0,34542329_12.2.1.4.0,35965629_12.2.1.4.0,39246821_12.2.1.4.0,39487427_12.2.1.4.0,39574504_12.2.1.4.0,39567777_12.2.1.4.0,34065178_12.2.1.4.0,39671832_12.2.1.4.0,38829855_12.2.1.4.0,33404495_12.2.1.4.0,32827327_12.2.1.4.0,31713053_12.2.1.4.0
      ```
...
1. Create the Oracle SOA Suite image:
    ...
    # Update OPatch and apply WebLogic patches
        COPY --chown=oracle:root p28186730_1394222_Generic.zip /tmp/imagetool/opatch/
        RUN cd /tmp/imagetool/opatch \
        && /u01/jdk/bin/jar -xf /tmp/imagetool/opatch/p28186730_1394222_Generic.zip \
...
1. Enter the following command to add the OPatch patch to the WebLogic Image Tool cache:
   ...
   ```bash
   $ imagetool cache addEntry --key 28186730_13.9.4.2.22 --value <download location>/p28186730_1394222_Generic.zip
   ```
...
1. Provide the following arguments to the WebLogic Image Tool `update` command:
    ...
    * `--tag` - Specify the new tag to be applied for the image being built.
    ...
    $ imagetool update --fromImage soasuite:12.2.1.4 --chown oracle:root --tag=soasuite:12.2.1.4-30761841 --patches=30761841_12.2.1.4.0 --opatchBugNumber=28186730_13.9.4.2.22
      ...
      Oracle Interim Patch Installer version 13.9.4.2.22
    ...
    $ imagetool update --fromImage soasuite:12.2.1.4 --chown oracle:root --tag=soasuite:12.2.1.4-30761841 --patches=30761841_12.2.1.4.0 --opatchBugNumber=28186730_13.9.4.2.22 --dryRun
    ...
    [INFO ] Using patch 28186730_13.9.4.2.22 from cache: <downloaded-patches-location>/p28186730_1394221_Generic.zip
    [WARNING] skipping patch conflict check, no support credentials provided
    [WARNING] No credentials provided, skipping validation of patches
    [INFO ] Using patch 30761841_12.2.1.4.0 from cache: <downloaded-patches-location>/p30761841_122140_Generic.zip
