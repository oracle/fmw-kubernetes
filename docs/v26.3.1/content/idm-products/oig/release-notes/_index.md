...
| Date | Version | Change |
| --- | --- | --- |
| July, 2026 | 26.3.1 | Supports Oracle Identity Governance 12.2.1.4 domain deployment using the July 2026 container image which contains the July Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | + Support for WebLogic Kubernetes Operator 4.3.12.|
| | | If upgrading to July 26 (26.3.1) from January 25 (25.1.1), you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.3.12|
| | | 2. Patch the OIG container image to July 26|
| | | 3. If you are upgrading to Kubernetes 1.30+ or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to July 26 (26.3.1) from October 22 (22.4.1) or later, you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.3.12|
| | | 2. Patch the OIG container image to July 26|
| | | 3. If you are upgrading to Kubernetes 1.30+ or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to July 26 (26.3.1) from a release prior to October 22 (22.4.1), you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.3.12|
| | | 2. Patch the OIG container image to July 26|
| | | 3. Upgrade the Ingress|
| | | 4. Upgrade Elasticsearch and Kibana|
| | | See [Patch and Upgrade](../patch-and-upgrade) for these instructions.
| April, 2026 | 26.2.1 | Supports Oracle Identity Governance 12.2.1.4 domain deployment using the April 2026 container image which contains the April Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | This release contains changes to support Traefik along with support for the April Patch Set Update.
| | | If upgrading to April 26 (26.2.1) from January 25 (25.1.1), you must upgrade the following in order:
| | | 1. Patch the OIG container image to April 26|
...
| | | If upgrading to April 26 (26.2.1) from October 22 (22.4.1) or later, you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OIG container image to April 26|
...
| | | If upgrading to April 26 (26.2.1) from a release prior to October 22 (22.4.1), you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OIG container image to April 26|
| | | 3. Upgrade the Ingress|
| | | 4. Upgrade Elasticsearch and Kibana|
| | | See [Patch and Upgrade](../patch-and-upgrade) for these instructions.
| January, 2026 | 26.1.1 | Supports Oracle Identity Governance 12.2.1.4 domain deployment using the January 2026 container image which contains the January Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | This release contains no changes other than support for the October Patch Set Update.
| | | If upgrading to January 26 (26.1.1) from January 25 (25.1.1), you must upgrade the following in order:
| | | 1. Patch the OIG container image to Janurary 26|
| | | 2. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to January 26 (26.1.1) from October 22 (22.4.1) or later, you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OIG container image to January 26|
| | | 3. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to January 26 (26.1.1) from a release prior to October 22 (22.4.1), you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OIG container image to January 26|
| | | 3. Upgrade the Ingress|
| | | 4. Upgrade Elasticsearch and Kibana|
| | | See [Patch and Upgrade](../patch-and-upgrade) for these instructions.
| October, 2025 | 25.4.1 | Supports Oracle Identity Governance 12.2.1.4 domain deployment using the October 2025 container image which contains the October Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | This release contains no changes other than support for the October Patch Set Update.
| | | If upgrading to October 25 (25.4.1) from January 25 (25.1.1), you must upgrade the following in order:
| | | 1. Patch the OIG container image to October 25|
| | | 2. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to October 25 (25.4.1) from October 22 (22.4.1) or later, you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OIG container image to October 25|
| | | 3. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to October 25 (25.4.1) from a release prior to October 22 (22.4.1), you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OIG container image to October 25|
| | | 3. Upgrade the Ingress|
| | | 4. Upgrade Elasticsearch and Kibana|
