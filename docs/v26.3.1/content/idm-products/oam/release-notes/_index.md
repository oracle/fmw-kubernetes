...
| Date | Version | Change |
| --- | --- | --- |
| July, 2026 | 26.3.1 | Supports Oracle Access Management 12.2.1.4 domain deployment using the July 2026 container image which contains the July Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | This release contains the following changes:
| | | + Support for WebLogic Kubernetes Operator 4.3.12.|
| | | If upgrading to July 26 (26.3.1) from April 26 (26.2.1) or later, you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.3.12|
| | | 2. Patch the OAM container image to July 26|
...
| April, 2026 | 26.2.1 | Supports Oracle Access Management 12.2.1.4 domain deployment using the April 2026 container image which contains the April Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | This release contains changes for Traefik support along with support for the April Patch Set Update.
| | | If upgrading to April 26 (26.2.1) from January 25 (25.1.1) or later, you must upgrade the following in order:
| | | 1. Patch the OAM container image to April 26|
...
| | | 2. Patch the OAM container image to April 26|
...
| | | 4. Upgrade Elasticsearch and Kibana|
| | | See [Patch and Upgrade](../patch-and-upgrade) for these instructions.
| January, 2026 | 26.1.1 | Supports Oracle Access Management 12.2.1.4 domain deployment using the January 2026 container image which contains the January Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | This release contains no changes other than support for the January Patch Set Update.
| | | If upgrading to January 26 (26.1.1) from January 25 (25.1.1) or later, you must upgrade the following in order:
| | | 1. Patch the OAM container image to January 26|
| | | 2. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to January 26 (26.1.1) from October 22 (22.4.1) or later, you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OAM container image to January 26|
| | | 3. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
...
| | | 4. Upgrade Elasticsearch and Kibana|
| | | See [Patch and Upgrade](../patch-and-upgrade) for these instructions.
| October, 2025 | 25.4.1 | Supports Oracle Access Management 12.2.1.4 domain deployment using the October 2025 container image which contains the October Patch Set Update (PSU) and other fixes released with the Critical Patch Update (CPU) program.|
| | | This release contains no changes other than support for the October Patch Set Update.
| | | If upgrading to October 25 (25.4.1) from January 25 (25.1.1) or later, you must upgrade the following in order:
| | | 1. Patch the OAM container image to October 25|
| | | 2. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
| | | If upgrading to October 25 (25.4.1) from October 22 (22.4.1) or later, you must upgrade the following in order:
| | | 1. WebLogic Kubernetes Operator to 4.2.10|
| | | 2. Patch the OAM container image to October 25|
| | | 3. If you are upgrading to Kubernetes 1.29 or later, you must upgrade the ingress. See [Upgrading the ingress](../patch-and-upgrade/upgrade-an-ingress).
