...
| `soaClusterName` | Name of the SOA WebLogic Server cluster instance to generate for the domain. By default, the cluster name is `soa_cluster`. This configuration parameter is applicable only for soa, soaess, soaosb, and soaessosb domain types.| `soa_cluster` |
| `osbClusterName` | Name of the Oracle Service Bus WebLogic Server cluster instance to generate for the domain. By default, the cluster name is `osb_cluster`. This configuration parameter is applicable only for osb, soaosb, and soaessosb domain types.| `osb_cluster` |
...
| `domainType` | Type of the domain. Mandatory input for Oracle SOA Suite domains. You must provide one of the supported domain type values: `soa` (deploys a SOA domain),`osb` (deploys an Oracle Service Bus domain),`soaess` (deploys a SOA domain with Oracle Enterprise Scheduler),`soaosb` (deploys a domain with SOA and Oracle Service Bus), and `soaessosb` (deploys a domain with SOA, Oracle Service Bus, and Oracle Enterprise Scheduler). | `soa`
