...
if [[ ${wlsMonitoringExporterTowcpCluster}  == "true" ]];
then
  update_wls_exporter_war wcp ${wcpManagedServerPort}
fi
if [[ ${wlsMonitoringExporterTowcpPortletCluster}  == "true" ]];
then
  update_wls_exporter_war wcpPortlet ${wcpPortletManagedServerPort}
fi
