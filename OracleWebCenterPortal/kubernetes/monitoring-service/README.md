# Monitor the OracleWebCenterPortal instance using Prometheus and Grafana
Using the `WebLogic Monitoring Exporter` you can scrape runtime information from a running OracleWebCenterPortal instance and monitor them using Prometheus and Grafana.
...
Alternatively, you can run the helper script `setup-monitoring.sh` available at `${WORKDIR}/monitoring-service` to setup the monitoring for OracleWebCenterPortal domain. 
