...
function list_mods() {
    yq eval '.environments[0].modules[].module' "${cfgFile}"
}
...
function nameOf() {
    #local idx=$1
    #yq e ".environments[0].modules[\"${idx}\"].name" "${cfgFile}"
...
# Construct output JSON using jq for formatting
output=$(jq -n \
    --arg envName "${envName}" \
    --arg k8sName "${k8sName}" \
    --arg ocneVer "${ocneVer}" \
    --arg ociKeyPath "${ociKeyPath}" \
    --arg kvrtConfig "${kvrtConfig}" \
    --arg error "${error}" \
    '{ "environment_name": $envName, "kubernetes_name": $k8sName, "version": $ocneVer, "oci_api_key_path": $ociKeyPath, "kubevirt_config": $kvrtConfig, "error": $error }')
