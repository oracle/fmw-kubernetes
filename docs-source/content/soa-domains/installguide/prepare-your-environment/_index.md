...
1. Pull the operator image:
    ...
    ```bash
    $ docker pull ghcr.io/oracle/weblogic-kubernetes-operator:4.2.4
    ```
...
1. Download from My Oracle Support:
    - Download patch [36825148](https://support.oracle.com/epmos/faces/ui/patch/PatchDetail.jspx?patchId=36825148) for Oracle Linux 7 based container image or [36825236](https://support.oracle.com/epmos/faces/ui/patch/PatchDetail.jspx?patchId=36825236) for Oracle Linux 8 based container image from My Oracle Support (MOS).
...
In the following example commands to install the WebLogic Kubernetes Operator, `opns` is the namespace and `op-sa` is the service account created for the operator:
  ...
  $ helm install weblogic-kubernetes-operator weblogic-operator/weblogic-operator --version 4.2.4 --namespace opns --set serviceAccount=op-sa --set "javaLoggingLevel=FINE" --wait
  ```
...
#### Create a Kubernetes secret with domain credentials
  ...
  For more details, see [this document](https://github.com/oracle/weblogic-kubernetes-operator/blob/v4.2.4/kubernetes/samples/scripts/create-weblogic-domain-credentials/README.md).
