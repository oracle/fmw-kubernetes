...
weight: 2
pre: "<b>ii. </b>"
description: "Configure Design Console with Traefik(SSL)."
---

Configure an Traefik ingress (SSL) to allow Design Console to connect to your Kubernetes cluster.
...
If you haven't already configured an Traefik ingress controller (SSL) for OIG, follow [Using an Ingress with Traefik (SSL)](../../../configure-ingress/traefik/ingress-traefik-setup-for-oig-domain-setup-on-k8s-ssl).

Make sure you know the hostname and port for the Traefik ingress controller before proceeding e.g `https://${HOSTNAME}:${PORT}`. Also make sure you know the Kubernetes secret for SSL that was generated e.g `governancedomain-tls-cert`.
...
1. Setup routing rules by running the following commands:
   ...
   Edit `values.yaml` and ensure that `type`: `TRAEFIK`, `tls: SSL` is set. Change `domainUID:` and `secretName:` to match the values for your `<domain_uid>` and your SSL Kubernetes secret, for example:
...
1. Import the CA certificate into the java keystore
   ...
   If in [Generate SSL Certificate](../../../configure-ingress/ingress-traefik-setup-for-oig-domain-setup-on-k8s-ssl#generate-a-ssl-certificate) you requested a certificate from a Certificate Authority (CA), then you must import the CA certificate (e.g cacert.crt) that signed your certificate, into the java truststore used by Design Console.

   If in [Generate SSL Certificate](../../configure-ingress/ingress-traefik-setup-for-oig-domain-setup-on-k8s-ssl#generate-a-ssl-certificate) you generated a self-signed certicate (e.g tls.crt), you must import the self-signed certificate into the java truststore used by Design Console.
...
1. Copy the Ingress CA certificate into the container
   ...
   If in [Generate SSL Certificate](../../../configure-ingress/ingress-traefik-setup-for-oig-domain-setup-on-k8s-ssl#generate-a-ssl-certificate) you requested a certificate from a Certificate Authority (CA), then you must copy the CA certificate (e.g cacert.crt) that signed your certificate, into the container

   If in [Generate SSL Certificate](../../../configure-ingress/ingress-traefik-setup-for-oig-domain-setup-on-k8s-ssl#generate-a-ssl-certificate) you generated a self-signed certicate (e.g tls.crt), you must copy the self-signed certificate into the container
