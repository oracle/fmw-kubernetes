...
1. If using Oracle Container Registry or your own container registry for your OIG container image, run the following command to create a helper pod to run RCU:
   ...
   ```bash
   $ kubectl run --image=container-registry.oracle.com/middleware/oig_cpu:12.2.1.4-jdk8-ol8-<October`25> --image-pull-policy="IfNotPresent" --overrides='{"apiVersion": "v1","spec":{"imagePullSecrets": [{"name": "orclcred"}]}}' helper -n oigns -- sleep infinity
   ```
   ...
   ```bash
   $ kubectl run helper --image oracle/oig:12.2.1.4-jdk8-ol8-<October`25> -n oigns -- sleep infinity
   ```
