...
1. [Install the OUD Docker Image](#install-the-oud-docker-image)
...
### Install the OUD Docker Image

You can deploy [OUD docker images](https://github.com/oracle/docker-images/tree/master/OracleUnifiedDirectory) in the following ways:

1. Download a pre-built OUD Docker image from [My Oracle Support](https://support.oracle.com).  This image contains the latest bundle patch and one-off patches for Oracle Unified Directory 12.2.1.4.0.
1. Build your own OUD Docker image using the WebLogic Image Tool. Oracle recommends using the WebLogic Image tool to build your own OUD 12.2.1.4.0 image along with the latest Bundle Patch and any additional patches that you require. For more information, see [Building an OUD Docker image using Image Tool](https://github.com/oracle/docker-images/tree/master/OracleUnifiedDirectory/imagetool/12.2.1.4.0).

Choose one of these options based on your requirements.

{{% notice note %}}
The OUD Docker image must be installed on the master node and each of the worker nodes in your Kubernetes cluster. Alternatively you can place the image in a docker registry that your cluster can access.
{{% /notice %}}

After installing the OUD docker image run the following command to make sure the image is installed correctly on the master and worker nodes:
...
Oracle Unified Directory deployment on Kubernetes leverages deployment scripts provided by Oracle for creating OUD containers using samples or Helm charts provided.  To deploy OUD on Kubernetes you should set up the deployment scripts on the **master** node as below:
...
From the directory you created, download the OUD deployment scripts from the OUD [repository](https://github.com/oracle/fmw-kubernetes.git).

```
$ git clone https://github.com/oracle/fmw-kubernetes.git
```

You can now use the deployment scripts from <Work Directory>/fmw-kubernetes/OracleUnifiedDirectory/kubernetes/samples/scripts/ to set up the OUD environments as further described in this document.
