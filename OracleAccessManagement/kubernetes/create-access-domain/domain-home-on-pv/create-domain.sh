...
function createDomainHome {
  ...
  CONTAINER_NAME="create-oam-infra-domain-job"
  ...
  if [ -n "${domainType}" ]; then
    ...
    if [ "${domainType}" == "oam2" ] || [ "${domainType}" == "oam3" ]; then
      echo "domainType is: ${domainType}"
      # Appends new cluster and rename it oam
    ...
    sed -i -e "s:%DOMAIN_TYPE%:oam:g" ${createJobOutput}
    sed -i -e "/- clusterName:/a ${PRECREATE_SERVICE}" ${dcrOutput}
  fi
