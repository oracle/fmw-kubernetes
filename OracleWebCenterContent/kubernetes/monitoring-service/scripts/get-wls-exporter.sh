...
if [[ ${wlsMonitoringExporterToibrCluster}  == "true" ]];
then
  update_wls_exporter_war ibr ${ibrManagedServerPort}
fi
if [[ ${wlsMonitoringExporterToucmCluster}  == "true" ]];
then
  update_wls_exporter_war ucm ${ucmManagedServerPort}
fi
if [[ ${wlsMonitoringExporterToipmCluster}  == "true" ]];
then
  update_wls_exporter_war ipm ${ipmManagedServerPort}
fi
if [[ ${wlsMonitoringExporterTocaptureCluster}  == "true" ]];
then
  update_wls_exporter_war capture ${captureManagedServerPort}
fi
if [[ ${wlsMonitoringExporterTowccadfCluster}  == "true" ]];
then
  update_wls_exporter_war wccadf ${wccadfManagedServerPort}
fi
