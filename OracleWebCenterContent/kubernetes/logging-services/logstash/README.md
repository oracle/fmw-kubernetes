## Publish  OracleWebCenterContent server and diagnostics logs into Elasticsearch
...
Create logstash configuration file. The logstash configuration file can be loaded from a volume. 
    ```bash
    $ kubectl cp logstash.conf  wccns/wccinfra-adminserver:/u01/oracle/user_projects/domains --namespace wccns
    ```
