...
check_stopped $OAANS $OAA_DEPLOYMENT-cache-proxy-0
check_stopped $OAANS $OAA_DEPLOYMENT-cache-rest-0

echo "Deleting Webhook"
kubectl delete mutatingwebhookconfigurations coherence-operator-mutating-webhook-configuration -n $OAANS >> $LOG 2>&1
kubectl delete validatingwebhookconfigurations coherence-operator-validating-webhook-configuration -n $OAANS >> $LOG 2>&1
kubectl delete validatingwebhookconfigurations externalip-validation-validating-webhook-configuration -n $OAANS >> $LOG 2>&1
...
echo "Deleting Coherence Operator"
oaa_mgmt "helm uninstall coherence-operator -n $OAACONS" >> $LOG 2>&1
...
kubectl delete rolebinding -n $OAACONS coherence-rolebinding >> $LOG 2>&1
...
kubectl delete role coherence-ns-role -n $OAACONS >> $LOG 2>&1
...
kubectl delete namespace $OAACONS >> $LOG 2>&1
