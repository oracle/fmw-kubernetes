...
update_db_connection()
{
    ...
    cd $PRIMARY_BASE/domains/$OIG_DOMAIN_NAME/config
    db_files=$(grep -rl "$OIG_REMOTE_SCAN" . | grep -v backup)
    if [ "$db_files" = "" ]
    then
    ...
    do 
       echo "Changing scan address from $OIG_REMOTE_SCAN to $OIG_LOCAL_SCAN in file $file"
       sed -i "s/$OIG_REMOTE_SCAN/$OIG_LOCAL_SCAN/g" $file
       if [ ! "$OIG_REMOTE_SERVICE" = "$OIG_LOCAL_SERVICE" ]
       then
           echo "Changing service from $OIG_REMOTE_SERVICE to $OIG_LOCAL_SERVICE in file $file"
           sed -i "s/$OIG_REMOTE_SERVICE/$OIG_LOCAL_SERVICE/g" $file
       fi
    done
...
PRIMARY_BASE=/u01/primary_oigpv
DR_BASE=/u01/dr_oigpv
check_backup_running
