...
SHUT_POLICY_SERVER_COUNT=0
new_step
if [ $STEPNO -gt $PROGRESS ]
then
    scale_cluster $OAMNS $OAM_DOMAIN_NAME policy-cluster $SHUT_POLICY_SERVER_COUNT
    update_progress
fi
...
if [ $STEPNO -gt $PROGRESS ]
then
    validate_scaling $OAMNS $OAM_DOMAIN_NAME policy-cluster $SHUT_POLICY_SERVER_COUNT
    update_progress
fi
