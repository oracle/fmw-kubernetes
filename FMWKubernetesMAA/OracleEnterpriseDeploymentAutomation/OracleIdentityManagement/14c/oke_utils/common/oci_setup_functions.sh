...
setup_kubectl() {
   ...
   if [[ $STEPNO -gt $PROGRESS ]]; then
     ...
     #  execute "$cmd"
     #fi
     KUBECTL_VERSION=$(curl https://cdn.dl.k8s.io/release/`echo $OKE_CLUSTER_VERSION | awk -F\v '{ print $2}' | awk -F\. '{print "latest-"$1"."$2".txt"}'` -sk --connect-timeout 30 )
     cmd="ssh -q -o \"StrictHostKeyChecking no\" -i $SSH_ID_KEYFILE opc@$BASTIONIP \"curl -LO https://storage.googleapis.com/kubernetes-release/release/$KUBECTL_VERSION/bin/linux/amd64/kubectl --connect-timeout 30 --silent --show-error \""
     execute "$cmd"
     print_msg end
   fi
...
install_webhost_packages() {
   IP=$1
   PACKAGE_LIST="libXrender libXtst xauth xterm nc libaio-devel* compat-libstdc++-* compat-libcap* gcc-c++-* ksh* libnsl* xorg-x11-utils* "
   for package in $PACKAGE_LIST
   do
     STEPNO=$((STEPNO+1))
     if [[ $STEPNO -gt $PROGRESS ]]; then
