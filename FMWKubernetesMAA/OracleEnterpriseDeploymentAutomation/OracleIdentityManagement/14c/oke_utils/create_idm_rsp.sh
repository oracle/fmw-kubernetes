...
if test -f $DIRNAME/responsefile/$1 ; then
  ...
  IDM_PWD_TEMPLATE=$DIRNAME/../responsefile/.idmpwds
...
for (( i=1; i < $OKE_NODE_POOL_SIZE; ++i ))
 do
   K8_WORKER_LABEL="K8_WORKER_HOST$i"
   if [[ "$i" -lt 3 ]]; then
     replace_variable "$K8_WORKER_LABEL" "${workers[i]}" "$IDM_TEMPLATE"
   else
     j=$((i - 1))
     checkLabel="K8_WORKER_HOST$j"
     sed -i "/^$checkLabel=.*/a $K8_WORKER_LABEL=\"${workers[i]}\"" $IDM_TEMPLATE
   fi
 done

for (( i=1; i <= $WEBHOST_SERVERS; ++i ))
 do
   OHS_HOST_LABEL="OHS_HOST$i"
   WEBHOST_LABEL="webhost$i"
   if [[ "$i" -lt 3 ]]; then
      replace_variable "$OHS_HOST_LABEL" "${!WEBHOST_LABEL}" "$IDM_TEMPLATE"
   else
      j=$((i - 1))
      checkLabel="OHS_HOST$j"
      sed -i "/^$checkLabel=.*/a $OHS_HOST_LABEL=\"${!WEBHOST_LABEL}\"" $IDM_TEMPLATE
   fi
 done
