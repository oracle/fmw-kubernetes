...
edit_domain_creation_file()
{
     ...
     then
        ...
        replace_value2 appVersion 14c $filename
...
add_image_wdt()
{
     ...
     cp $TEMPLATE_DIR/oigWdtDomain.sedfile $WORKDIR
     update_variable "<OIM_MEMORY>" "$OIM_MEMORY" $WORKDIR/oigWdtDomain.sedfile
     update_variable "<OIM_MAX_MEMORY>" "$OIM_MAX_MEMORY" $WORKDIR/oigWdtDomain.sedfile
     update_variable "<OIM_MAX_CPU>" "$OIM_MAX_CPU" $WORKDIR/oigWdtDomain.sedfile
     update_variable "<OIM_CPU>" "$OIM_CPU" $WORKDIR/oigWdtDomain.sedfile
     update_variable "<SOA_MEMORY>" "$SOA_MEMORY" $WORKDIR/oigWdtDomain.sedfile
     update_variable "<SOA_MAX_MEMORY>" "$SOA_MAX_MEMORY" $WORKDIR/oigWdtDomain.sedfile
     update_variable "<SOA_MAX_CPU>" "$SOA_MAX_CPU" $WORKDIR/oigWdtDomain.sedfile
     update_variable "<SOA_CPU>" "$SOA_CPU" $WORKDIR/oigWdtDomain.sedfile
     sed -i -f $WORKDIR/oigWdtDomain.sedfile $filename
     print_status $?
     ET=`date +%s`
...
create_connector_files()
{
     ...
     if  [ "${OAM_OIG_INTEG,,}" = "true" ]; then
       SSO_ENABLED_FLAG=true
     else
       SSO_ENABLED_FLAG=false
     fi
     ...
     update_variable "<SSO_ENABLED_FLAG>" $SSO_ENABLED_FLAG $WORKDIR/oamoig.sedfile
